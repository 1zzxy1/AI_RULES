...
**Git**：需要读/改工程时先 `git remote -v` 确认远程；没有则主动询问是否创建，确认前不擅自创建或推送。每处改动改完即独立提交。`--amend`/`--force`/`--no-verify` 仅在用户明确要求时用。提交信息用 Conventional Commits（英文）。Git/代码/测试是事实来源。**未经用户要求不主动提交。**
